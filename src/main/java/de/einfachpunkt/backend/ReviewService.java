package de.einfachpunkt.backend;
...
import de.einfachpunkt.ui.encoders.LocalDateToStringEncoder;
...
//                NewsModel newsModel = CategoryService.getInstance()
...
//                review.setNewsModel(newsModel);
    ...
    private String filterTextOf(Review review) {
        ...
        String filterableText = Stream
                .of(review.getName(),
//                        review.getNewsModel() == null ? StaticData.UNDEFINED
//                                : review.getNewsModel().getName(),
...
//        NewsModel newsModel = dto.getNewsModel();
        FeedItem newsModel = null;

        if (newsModel != null) {
            // The case when the newsModel is new (not persisted yet, thus
            // has null id) is not handled here, because it can't currently
            // occur via the UI.
            // Note that FeedItem.UNDEFINED also gets mapped to null.
//            newsModel = CategoryService.getInstance()
//                    .findCategoryById(newsModel.getId()).orElse(null);
        }
        if (entity == null) {
